...
public:
	...
	bool FindMarkersInROI(std::shared_ptr<cv::SimpleBlobDetector::Params> params, Mat src, int roiSize);
	bool FindMarkers(std::shared_ptr<cv::SimpleBlobDetector::Params> params, Mat src);
	...
	bool FindCenterMarker(std::shared_ptr<cv::SimpleBlobDetector::Params> params, Mat src,
		vector<Point2f> pts, Point2f &centerPt);
};
